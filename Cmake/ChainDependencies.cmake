...
function(chx_show_dependency DEP NAME)

	get_property(DISPLAYED GLOBAL PROPERTY CHX_${DEP}_DISPLAYED)

	if (NOT DISPLAYED)

		set_property(GLOBAL PROPERTY CHX_${DEP}_DISPLAYED TRUE)
...
if (DEFINED MSVC)
	...
	# this must be set to point to the same directory as $CHX_DEPENDENCY_INSTALL_DIR in /extdep directory



	if (CMAKE_CXX_COMPILER_VERSION VERSION_LESS 19.0.0)

		set (CHX_DEPENDENCY_INSTALL_DIR "${CMAKE_CURRENT_LIST_DIR}/../extdep/install/windows/x64")

	else()

		get_filename_component(DEPS_DIR "${CMAKE_CURRENT_LIST_DIR}/../deps/install" ABSOLUTE)

		set(CHX_DEPENDENCY_INSTALL_DIR
	...
	set (CMAKE_PREFIX_PATH ${CHX_DEPENDENCY_INSTALL_DIR} ${CMAKE_PREFIX_PATH})
...
set(CHX_CMAKE_DIR ${CMAKE_CURRENT_LIST_DIR})

set(CHX_SCRIPTS_DIR ${CHX_CMAKE_DIR}/scripts)
...
#message(STATUS "CMake Helper Path: ${CHX_CMAKE_DIR}")

#message(STATUS "CMake Script Path: ${CHX_SCRIPTS_DIR}")
...
chx_show_dependency(Boost boost)
...
function(chx_use TARGET REQUIRED)

	if (NOT TARGET ${TARGET})

		message(FATAL_ERROR "chx_use called for non existing target ${TARGET}")
	...
	foreach(MODULE ${ARGN})
		...
		if (NOT TARGET_APPLIED)
			...
			chx_apply(${TARGET} ${REQUIRED} ${MODULE_SUB})
